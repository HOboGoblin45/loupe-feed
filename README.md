...
[![Refresh catalog](https://github.com/HOboGoblin45/loupe-feed/actions/workflows/refresh-catalog.yml/badge.svg)](https://github.com/HOboGoblin45/loupe-feed/actions/workflows/refresh-catalog.yml)
[![Archive watchdog](https://github.com/HOboGoblin45/loupe-feed/actions/workflows/archive-watchdog.yml/badge.svg)](https://github.com/HOboGoblin45/loupe-feed/actions/workflows/archive-watchdog.yml)
...
The daily commit history of `catalog.json` *is* the price and availability
record, and it is the one thing here that cannot be re-acquired — a day that was
not observed cannot be observed later, at any price. So it is measured rather
than assumed:

- **[ARCHIVE_STATUS.md](ARCHIVE_STATUS.md)** — regenerated by
  `loupe-feed/archive_integrity.py` on every catalog commit and folded into that
  same commit: days, coverage, longest unbroken run, and every missing day.
  A machine-made continuity record, not a claim.
- **Watchdog** — `.github/workflows/archive-watchdog.yml` re-measures at 11:00 and
  23:00 UTC on a schedule of its own and goes red (badge above, plus an issue and
  an email) if the newest snapshot is more than 36 hours old. It deliberately
  shares no step, script or secret with the build, so the failure it watches for
  cannot take it down too.
- **Heartbeat** — the daily build fails rather than committing a catalog that is
  unchanged apart from its timestamp, or one whose product count moved
  implausibly. A green run is supposed to mean a day was added.
- **Backup** — `.github/workflows/archive-backup.yml` publishes a full `git
  bundle` of the repo plus a distilled price/availability series as GitHub
  Release assets every Sunday. Releases survive a rewritten history; the git
  object store is the thing being insured.

There is one gap, `2026-07-25` → `2026-07-28`, and the reason is recorded in
`ARCHIVE_STATUS.md`. All of the above exists because of it.
